...
class Fairy:
    def __init__(self, video_name = 'Kettlebells-Emily'):
        ...
        self.video_output_root_path = Path(config["PATH"]["video_output_root_path"])
        if self.video_output_root_path.exists() == False:
            subprocess.check_output(["mkdir", self.video_output_root_path], 
                                    stderr=subprocess.STDOUT)

        self.data_output_root_path = Path(config["PATH"]["data_output_root_path"])
        ...
        print(f"video path : {self.video_path}, model path : {self.model_path}")
    ...
    def model_inference_video(self):
        ...
        while cap.isOpened():
            ...
            width = height
            image_resize = cv2.resize(image_rgb, (256, 256), interpolation=cv2.INTER_AREA)
        ...
        out = cv2.VideoWriter(config["PATH"]["output_inference_path"], fourcc, fps, frame_size)
        cap = cv2.VideoCapture(self.video_target_path)
        count = 0
        ...
        while cap.isOpened():
            ...
            for keypoint in body_keypoint:
                y_coordinate = int(keypoint[0] * height); x_coordinate = int(keypoint[1] * width)
                score = keypoint[2]
                if score > 0.8:
                    cv2.circle(frame, (x_coordinate, y_coordinate), 1, (255,0,0), 5)
                elif score > 0.4:
                    cv2.circle(frame, (x_coordinate, y_coordinate), 1, (255,255,0), 5)
                else:
                    cv2.circle(frame, (x_coordinate, y_coordinate), 1, (0,0,255), 5)

            cv2.putText(frame, str(count), (30, 30), cv2.FONT_HERSHEY_SIMPLEX,1, (255, 255, 255), 1, cv2.LINE_AA)
            out.write(frame)
    ...
    def modify_pickle(self, 
        ...
        for index, keypoint in enumerate(body_keypoint_list[frame_index]):
            ...
            if score > 0.8:
                cv2.circle(frame, (x_coordinate, y_coordinate), 1, (255,0,0), _l)
            elif score > 0.4:
                cv2.circle(frame, (x_coordinate, y_coordinate), 1, (255,255,0), _l)
            else:
                cv2.circle(frame, (x_coordinate, y_coordinate), 1, (0,0,255), _l)

        y_coordinate = int(y_rate * height); x_coordinate = int(x_rate * width)
    ...
    def write_json_file(self, sample_json):
        ...
        for coach_keypoint in coach_keypoint_list:
            _ = tools.changeWholeBodyAngleLengthVectorDictionary(coach_keypoint.numpy())
    ...
    def simplify_json_file(self, json_path = 'datas_output/labels.json'):
        ...
        label_inverse_map = lambda x: label_index_list.index(x)
        ...
        for human_keypoint_label in data["focusOnHumanKeypoint"]:
            check_points = human_keypoint_label['checkPoints']
            for check_point in check_points:
                check_point['label_index'] = label_inverse_map(check_point['label_index'])
